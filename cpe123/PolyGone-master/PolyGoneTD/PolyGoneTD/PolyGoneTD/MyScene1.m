...
- (void)didEvaluateActions
{
    if(self.count%200==0)
    {
        for(int x = 0; x< [self.enemies count]; x++)
        {
            SquareEnemy *enemy = [self.enemies objectAtIndex:x];
            if(enemy.isOnFire)
            {
                [enemy removeOneLife];
            }
...
-(id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        self.backgroundColor = [SKColor colorWithRed:1 green:0.0 blue:0.0 alpha:0.8];
        self.physicsWorld.contactDelegate = self;
        
        EnemyPathS *path = [[EnemyPathS alloc] initPath];
        [self addChild:path];
        
        EnemyPathS *end = [[EnemyPathS alloc] initEnd];
        [self addChild:end];
...
- (SKNode*) addEnemy{
    ...
    SquareEnemy *enemy = [[SquareEnemy alloc] initWithDifficulty:self.player.difficulty inMap:2];
    [self addChild:enemy];
...
-(void)didBeginContact:(SKPhysicsContact *)contact
{
    ...
    if ((firstBody.categoryBitMask & bulletCategory) != 0)
    {
        ...
        //printf("It collided");
        self.currencyCount+=1;
        
        if(self.currencyCount%(self.player.difficulty+1)==0)
    ...
    else if ((firstBody.categoryBitMask & tower1Category) !=0)
    {
        ...
        FireBullet *bullet = [[FireBullet alloc] initWithDestination:enemyLoc WithLocation:towerLoc WithType:type];
