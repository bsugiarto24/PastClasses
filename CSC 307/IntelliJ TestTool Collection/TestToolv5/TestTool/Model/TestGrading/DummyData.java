...
public class DummyData {
    ...
    private static ArrayList<ActiveTest> makeDummyData() {
        ...
        for (int i = 0; i < 6; i++) {
            ((TrueFalse)tests.get(0).getStudentTests().get(i).getQuestions().get(0)).setStudentAnswer(true);
            ((TrueFalse)tests.get(0).getStudentTests().get(i).getQuestions().get(1)).setStudentAnswer(true);
            ((FreeResponse)tests.get(0).getStudentTests().get(i).getQuestions().get(2)).setStudentAnswer("A student's answer!");
            ((Coding)tests.get(0).getStudentTests().get(i).getQuestions().get(3)).setStudentAnswer("System.out.println(\"Hello World!\")");
        }
        return tests;
    }
