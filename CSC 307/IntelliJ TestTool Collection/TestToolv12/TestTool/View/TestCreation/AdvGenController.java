...
//        course.setItems(FXCollections.observableArrayList(CourseCollection.getInstance().getAllCourseList()));
//    }
//
//
//    private void updateSubjects(final Course newValue) {
//        for (int i = 0; i < subjVBox.getChildren().size(); i ++) {
//            ComboBox temp = (ComboBox) subjVBox.getChildren().get(i);
//            temp.setItems(FXCollections.observableArrayList(newValue.getSubjects()));
//        }
//    }
//
//    private void initializeListeners() {
//        course.valueProperty().addListener(new ChangeListener<Course>() {
//            @Override
//            public void changed(final ObservableValue<? extends Course> observable, final Course oldValue, final Course newValue) {
//                if (newValue != null) {
//                    for (int i = 0; i < subjVBox.getChildren().size(); i ++) {
//                        subjVBox.getChildren().get(i).setDisable(false);
//                    }
...
//    }
    ...
    public void updateSubj() {
        if (!course.getValue().equals("None")) {
            for(int i = 0; i < subjVBox.getChildren().size(); i++) {
                subjVBox.getChildren().get(i).setDisable(false);
                ComboBox<String> temp = (ComboBox) subjVBox.getChildren().get(i);

                Course getCourse = CourseCollection.getInstance().findCourse((String) course.getValue());
                Collection<Subject> subjs = getCourse.getSubjects();
    ...
    public void addRow() {
        ...
        newSubj.setDisable(true);
        //newSubj.getItems().addAll(Course.getInstance());
    ...
    private boolean hasErrors(){
        ...
        try {
            ...
            if (testLenType.getValue().equals(null) || testLenType.getValue().equals("None") ||
                    course.getValue() == null || course.getValue().equals("None")) {
            ...
            for (int i = 0; i < delVBox.getChildren().size(); i++) {
                ...
                TextField numQuesB = (TextField) numQuesVBox.getChildren().get(i);
                Integer.valueOf(numQuesB.getText());


                TextField diffB = (TextField) diffVBox.getChildren().get(i);
                Integer.valueOf(diffB.getText());
            ...
            alertUser("Sorry please make sure test length, number questions, and difficulty are integers.");
            return true;
        }
        catch(Exception e) {
