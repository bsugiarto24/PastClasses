...
public class StudentViewController implements Initializable {
    ...
    @FXML
    private ListView<String> listBoxSubjects;
    ...
    public void print(){
        ...
        if(listBoxStudents.getItems().size() > 0)
    ...
    public void testSelection(){
        ...
        if(listBoxTests.getItems().size() > 0)
    ...
    public void deleteSubject() {
        String subject = listBoxSubjects.getFocusModel().getFocusedItem();
        Subject delete = SubjectCollection.getInstance().findSubject(subject);

        //Subject delete = new Subject("subject");

        System.out.println("Deleting Sujbect: " + subject);

        course.removeSubject(delete);

        ObservableList<String> list = listBoxSubjects.getItems();
        list.remove(delete.getName());

        SubjectCollection.getInstance().deleteSubject(delete);

        listBoxSubjects.setItems(list);
    }
