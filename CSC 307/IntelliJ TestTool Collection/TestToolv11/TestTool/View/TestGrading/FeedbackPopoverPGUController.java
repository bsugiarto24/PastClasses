...
public class FeedbackPopoverPGUController implements Initializable {
    ...
    public FeedbackPopoverPGUController(final MainApp mainApp, final ActiveTest activeTest, final StudentTest studentTest, final Question question, final TestGradingQuestionController qc) {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        if (questionClass == FreeResponse.class) {
            FreeResponse fr = (FreeResponse) question;
            answerString = fr.getStudentAnswer();
            keywordString = fr.getFlags().toString();
            for (String keyword : fr.getFlags()) {
                totalKeyWords++;
                if (answerString.contains(keyword)) {
                    keyWordCount++;
                }
            }
        } else if (questionClass == Coding.class) {
            Coding cq = (Coding) question;
            answerString = cq.getStudentAnswer();
            keywordString = cq.getFlags().toString();
            for (String keyword : cq.getFlags()) {
                totalKeyWords++;
                if (answerString.contains(keyword)) {
                    keyWordCount++;
                }
        ...
        expectedKeywordsArea.setText(keywordString);

        Text keywordsFound = new Text(keyWordCount + "/" + totalKeyWords);
        foundKeywordsPane.getChildren().add(keywordsFound);
    }
